...
## Consequences
Implementation turns have workspace write access without network access. Proofs have no unsandboxed fallback: they receive workspace-scoped writes, disabled network, a scrubbed credential environment, connection-owned process lifetime, and bounds of three attempts, thirty minutes, and one MiB of output. Three failed receipts deterministically stop execution in Human Review instead of entering another agent or orchestrator retry. Instruction or repository drift fails closed; clean final proof and review receipts become stale after any edit or commit. PR state can invalidate handoff after publication. The control store is single-instance local state and is deliberately not a cross-host coordination protocol.
